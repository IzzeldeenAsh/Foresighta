...
export class MeetingsService {
  private apiUrl = `${environment.apiBaseUrl}/insighter/meeting/list`;
  ...
  updateMeetingStatus(meetingUuid: string, status: 'approved' | 'postponed', notes: string): Observable<any> {
    ...
    const url = `${environment.apiBaseUrl}/insighter/meeting/action/${meetingUuid}`;
  ...
  getAvailableHours(): Observable<AvailableHoursResponse> {
    const headers = this.getHeaders();
    const url = `${environment.apiBaseUrl}/insighter/meeting/available/hours`;
  ...
  rescheduleMeeting(meetingUuid: string, rescheduleData: RescheduleRequest): Observable<any> {
    const headers = this.getHeaders();
    const url = `${environment.apiBaseUrl}/insighter/meeting/reschedule/${meetingUuid}`;
  ...
  archiveMeeting(meetingUuid: string): Observable<any> {
    const headers = this.getHeaders();
    const url = `${environment.apiBaseUrl}/insighter/meeting/archive/${meetingUuid}`;
  ...
  getArchivedMeetings(page: number = 1, perPage: number = 10): Observable<MeetingResponse> {
    const headers = this.getHeaders();
    const url = `${environment.apiBaseUrl}/insighter/meeting/list`;
  ...
  getClientMeetingStatistics(): Observable<{ data: ClientMeetingStatistics }> {
    const headers = this.getHeaders();
    const url = `${environment.apiBaseUrl}/insighter/meeting/statistics`;
